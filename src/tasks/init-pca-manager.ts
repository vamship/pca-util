...
const installTillerCommands = [
    ...
    [
        ...
        [
            ...
            '  template:',
            '    spec:',
            '      serviceAccountName: pca-default-account',
            '      restartPolicy: Never',
            '      containers:',
            '      - name: installer',
            ...
            '            mountPath: /etc/pca-manager/tiller-certificate',
            '          - name: helm-ca-certificate',
            '            mountPath: /etc/pca-manager/helm-ca-certificate',
            '        command: [ "helm" ]',
            [
                '        args: [',
                [
                    ...
                    '"--tiller-tls"',
                    '"--tiller-tls-verify"',
                    '"--service-account=pca-helm-account"',
                    '"--tiller-tls-cert=/etc/pca-manager/tiller-certificate/tls.crt"',
                    '"--tiller-tls-key=/etc/pca-manager/tiller-certificate/tls.key"',
                    '"--tls-ca-cert=/etc/pca-manager/helm-ca-certificate/tls.crt"',
            ...
            '        - name: tiller-certificate',
            '          secret:',
            '            secretName: pca-tiller-certificate',
            '        - name: helm-ca-certificate',
            '          secret:',
            '            secretName: pca-helm-ca-certificate',
            'EOF'
        ].join('\n'),
        'END_SCRIPT'
    ].join('\n')
...
const installMetalLbCommands = [
    ...
    [
        ...
        [
            ...
            '            mountPath: /etc/pca-manager/helm-certificate',
            '          - name: helm-ca-certificate',
            '            mountPath: /etc/pca-manager/helm-ca-certificate',
            '        command: [ "/bin/sh", "-c" ]',
            [
                ...
                [
                    ...
                    'ln -s /etc/pca-manager/helm-certificate/tls.crt /root/.helm/cert.pem',
                    'ln -s /etc/pca-manager/helm-certificate/tls.key /root/.helm/key.pem',
                    'ln -s /etc/pca-manager/helm-ca-certificate/tls.crt /root/.helm/ca.pem',
            ...
            '        - name: helm-certificate',
            '          secret:',
            '            secretName: pca-helm-certificate',
            '        - name: helm-ca-certificate',
            '          secret:',
            '            secretName: pca-helm-ca-certificate',
            'EOF'
