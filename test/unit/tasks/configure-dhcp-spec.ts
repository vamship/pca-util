...
const _taskModule = _rewire('../../../src/tasks/configure-dhcp');

describe('[configure-dhcp task]', () => {
    ...
    describe('getTask()', () => {
        const expectedTitle = 'Configure DHCP';
        const subTaskList = [
            {
        ...
        subTaskList.forEach(({ title, commandCount, eatError }, index) => {
            describe(`[sub task: ${title}]`, () => {
                ...
                if (index === 0) {
                    it('should set the ctx.skipDhcpConfig=false if command execution fails', () => {
                        const sshClientMock = getSshClientMock();
                        const runMethod = sshClientMock.mocks.run;
                        const ctx = {
                            skipDhcpConfig: undefined
                        };
                        ...
                        return expect(ret).to.be.fulfilled.then(() => {
                            expect(ctx.skipDhcpConfig).to.be.false;
                        });
                    });

                    it('should set the ctx.skipDhcpConfig=true if command execution succeeds', () => {
                        const sshClientMock = getSshClientMock();
                        const runMethod = sshClientMock.mocks.run;
                        const ctx = {
                            skipDhcpConfig: undefined
                        ...
                        return expect(ret).to.be.fulfilled.then(() => {
                            expect(ctx.skipDhcpConfig).to.be.true;
                        });
                    });
                } else {
                    describe('[skip]', () => {
                        ...
                        it('should return false if ctx.skipDhcpConfig === false', () => {
                            const ret = _execSkip(undefined, {
                                skipDhcpConfig: false
                            });
                            expect(ret).to.be.false;
                        });

                        it('should return a message if ctx.skipDhcpConfig === true', () => {
                            const ret = _execSkip(undefined, {
                                skipDhcpConfig: true
                            });
